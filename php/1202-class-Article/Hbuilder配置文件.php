...
snippet '$this->success()' do |s|
    s.trigger = 'thiss'
    s.expansion = '\$this->success($0);'
  end
  
snippet '$this->error()' do |s|
    s.trigger = 'thise'
    s.expansion = '\$this->error($0);'
  end
  
 snippet '$this->assign()' do |s|
    s.trigger = 'thisa'
    s.expansion = '\$this->assign($0);'
  end
  
  snippet '$this->ajax()' do |s|
    s.trigger = 'thisj'
    s.expansion = '\$this->ajax($0);'
  end
 ...
 snippet 'return false' do |s|
  ...
  snippet '$this->display()' do |s|
    s.trigger = 'thisd'
    s.expansion = '\$this->display($0);'
  end
  
  snippet 'if(IS_POST)' do |s|
    s.trigger = 'ifp'
    s.expansion = 'if(IS_POST){
    p(Q("post."));
    //\$this->success();
}'
  end
  ...
  snippet 'p(Q("post."))' do |s|
    s.trigger = 'pq'
    s.expansion = 'p(Q("post."));'
  end
  
  snippet 'public function' do |s|
    s.trigger = 'publicf'
    s.expansion = 'public function $1(){
    View::make();
}'
  end
  
  snippet "header('Content-type:text/html;charset=utf-8');" do |s|
